...
/*   Updated: 2025/08/29 10:18:52 by seruff           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_window(t_data *data)
{
	...
	data->mlx = mlx_init();
	if (!data->mlx)
		exit_win(data, "Mlx init failed", 4);
	data->win = mlx_new_window(data->mlx, width, height, "CUB3D");
	if (!data->win)
		exit_win(data, "Window creation failed", 4);
	data->img.img = mlx_new_image(data->mlx, width,	height);
	if (!data->img.img)
		exit_win(data, "Mlx_new_imagefailed", 4);
	data->img.img_addr = mlx_get_data_addr(data->img.img, &data->img.bits,
			&data->img.line_length, &data->img.endian);	
	if (!data->img.img_addr)
		exit_win(data, "Mlx_get_data_addr failed", 4);
}
